#!/usr/bin/env python3
"""
Analyze anomalies in bank_medium dataset
"""
import pandas as pd
import numpy as np
...
def analyze_anomalies():
    # Load data
    transactions = pd.read_csv('bank_medium/bank_medium/transactions.csv')
    alert_transactions = pd.read_csv('bank_medium/bank_medium/alert_transactions.csv')
    sar_accounts = pd.read_csv('bank_medium/bank_medium/sar_accounts.csv')
    accounts = pd.read_csv('bank_medium/bank_medium/accounts.csv')

    print('=== BANK MEDIUM SUSPICIOUS TRANSACTIONS ANALYSIS ===')
    ...
    print('\n=== COMPARISON WITH BANK SMALL ===')
    print('Bank Small: 4,993 transactions, 62 suspicious (1.24%)')
    print(f'Bank Medium: {len(transactions)} transactions, {len(suspicious_tx)} suspicious ({len(suspicious_tx)/len(transactions)*100:.2f}%)')
    print('Key differences:')
    print('- Medium dataset has lower suspicious percentage (0.57% vs 1.24%)')
    print('- More transactions but fewer suspicious ones')
    print('- Different class imbalance challenge')
    ...
    print('7. Class Imbalance: More severe than bank_small (0.57% vs 1.24%)')
