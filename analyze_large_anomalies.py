#!/usr/bin/env python3
"""
Analyze anomalies in bank_large dataset
"""
import pandas as pd
import numpy as np
...
def analyze_anomalies():
    # Load data
    transactions = pd.read_csv('bank_large/transactions.csv')
    alert_transactions = pd.read_csv('bank_large/alert_transactions.csv')
    sar_accounts = pd.read_csv('bank_large/sar_accounts.csv')
    accounts = pd.read_csv('bank_large/accounts.csv')

    print('=== BANK LARGE SUSPICIOUS TRANSACTIONS ANALYSIS ===')
    ...
    print('\n=== TEMPORAL PATTERNS ===')
    suspicious_tx_copy = suspicious_tx.copy()
    suspicious_tx_copy['date'] = pd.to_datetime(suspicious_tx_copy['tran_timestamp']).dt.date
    daily_suspicious = suspicious_tx_copy.groupby('date').size()
    ...
    print('Bank Small: 4,993 transactions, 62 suspicious (1.24%)')
    print('Bank Medium: 15,482 transactions, 89 suspicious (0.57%)')
    print(f'Bank Large: {len(transactions)} transactions, {len(suspicious_tx)} suspicious ({len(suspicious_tx)/len(transactions)*100:.2f}%)')
    print('Key trends:')
    print('- Large dataset has the most severe class imbalance (0.24%)')
    print('- More transactions but fewer suspicious ones proportionally')
    print('- Most challenging dataset for ML models')
    print('- Represents realistic production-scale banking environment')

    print('\n=== SUMMARY OF ANOMALY CHARACTERISTICS ===')
    print('1. Pattern Type: Fan-in and Cycle patterns detected')
    print('2. Transaction Type: All suspicious transactions are TRANSFER type')
    print('3. Amount Range: Similar to normal transactions (evasion tactics)')
    ...
    print('7. Class Imbalance: Most severe across all datasets (0.24%)')
    print('8. Scale: Largest dataset representing production environment')
