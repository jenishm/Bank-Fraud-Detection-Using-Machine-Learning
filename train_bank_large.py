...
DATA_DIR = os.path.join(os.path.dirname(__file__), 'bank_large')
...
def _logreg_pipeline(X: pd.DataFrame) -> Pipeline:
    ...
    model = LogisticRegression(max_iter=500, class_weight='balanced')  # Increased iterations for large dataset
...
def _xgb_pipeline(X: pd.DataFrame) -> Pipeline:
    ...
    clf = xgb.XGBClassifier(
        n_estimators=500,  # Increased for large dataset
...
def main():
    ...
    save_artifacts('bank_large_logreg', logreg_model, logreg_metrics)

    xgb_model, xgb_metrics = train_xgboost(X, y)
    save_artifacts('bank_large_xgboost', xgb_model, xgb_metrics)
    ...
    if args.cv and args.cv > 1:
        ...
        with open(os.path.join(REPORTS_DIR, f'bank_large_logreg_cv{args.cv}.json'), 'w') as f:
        ...
        with open(os.path.join(REPORTS_DIR, f'bank_large_xgboost_cv{args.cv}.json'), 'w') as f:
