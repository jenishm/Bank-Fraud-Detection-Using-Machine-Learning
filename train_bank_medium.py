...
DATA_DIR = os.path.join(os.path.dirname(__file__), 'bank_medium', 'bank_medium')
...
def main():
    ...
    save_artifacts('bank_medium_logreg', logreg_model, logreg_metrics)

    xgb_model, xgb_metrics = train_xgboost(X, y)
    save_artifacts('bank_medium_xgboost', xgb_model, xgb_metrics)
    ...
    if args.cv and args.cv > 1:
        ...
        with open(os.path.join(REPORTS_DIR, f'bank_medium_logreg_cv{args.cv}.json'), 'w') as f:
        ...
        with open(os.path.join(REPORTS_DIR, f'bank_medium_xgboost_cv{args.cv}.json'), 'w') as f:
